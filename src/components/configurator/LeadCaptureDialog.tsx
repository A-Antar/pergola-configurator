...
export default function LeadCaptureDialog({ open, onOpenChange, config }: LeadCaptureDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // For now, log the lead. Backend integration comes later.
    const lead = {
      ...form,
      serviceType: 'patios',
      configJson: config,
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="bg-card border-border max-w-md">
          <div className="text-center py-8 space-y-4">
            ...
            <Button onClick={() => { setSubmitted(false); onOpenChange(false); }} variant="outline" className="mt-4">
              Close
            </Button>
          </div>
        </DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <div className="bg-secondary/50 rounded p-3 text-xs space-y-1">
            <p className="text-muted-foreground">
              Service: <span className="text-foreground">Patios — {config.material}, {config.shape}, {config.style.replace('-', ' ')}</span>
            </p>
            <p className="text-muted-foreground">
              Size: <span className="text-foreground">{config.width}m × {config.depth}m ({area.toFixed(1)} m²)</span>
            </p>
            <p className="text-muted-foreground">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              rows={2}
              placeholder="e.g. Replacing old timber pergola, need council approval..."
            />
